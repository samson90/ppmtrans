...
extern T UArray2b_new(int w, int h, int size, int b){
    T uarray2b;
    NEW(uarray2b);
    uarray2b->blockArray = UArray_new(((w - 1)/b + 1)*((h-1)/b + 1), size);
    uarray2b->blockSize = b;
    uarray2b->height = h;
    uarray2b->width = w;
    int i;
    for(i = 0; i < UArray_length(uarray2b->blockArray); i++){
        UArray_T *a = UArray_at(uarray2b->blockArray, i);
        *a = UArray_new(b * b, size);
    }
    return uarray2b;
}
...
extern void UArray2b_free(UArray2b_T* uarray2b){
    assert(uarray2b);
    int i;
    for (i = 0; i < UArray_length((*uarray2b)->blockArray); i++)
        UArray_free(UArray_at((*uarray2b)->blockArray, i));
...
void *UArray2b_at(T uarray2b, int i, int j){
    ...
    UArray_T * internal = UArray_at(uarray2b->blockArray, 
                                    (i/b)+(j/b)*((b+w-1)/b));
    printf("%i\n", UArray_length(*internal));
    printf("%i\n", b*(j%b)+i%b);
    return UArray_at(*internal, b*(j%b)+i%b);
}


void UArray2b_map(T uarray2b, void apply(int i, int j, T uarray2b, 
                  void *elem, void *c1), void *c1){
